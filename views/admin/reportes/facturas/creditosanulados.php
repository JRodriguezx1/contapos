  ...
  <div class="mx-auto grid min-w-0 max-w-screen-2xl gap-6 rounded-lg border border-slate-200 bg-gradient-to-b from-indigo-50 via-white to-white p-4 shadow-sm sm:p-6">
    <section class="grid min-w-0 grid-cols-1 items-center gap-5 rounded-lg border border-slate-200 bg-gradient-to-br from-violet-100 to-cyan-50 p-4 sm:p-6 md:grid-cols-[auto_minmax(0,1fr)] xl:grid-cols-[auto_minmax(0,1fr)_auto]">
      ...
      <div class="min-w-0">
        ...
        <h1 class="m-0 text-3xl font-extrabold leading-tight text-slate-900 sm:text-4xl">Creditos anulados</h1>
        <span class="mt-1 block text-lg leading-snug text-slate-500">Consulta creditos y separados que fueron cancelados o reversados.</span>
      </div>

      <div class="flex min-w-0 flex-wrap gap-4 md:col-span-2 xl:col-span-1 xl:justify-end" aria-label="Resumen de creditos anulados">
        <article class="flex min-w-72 flex-1 items-center gap-4 rounded-lg border border-slate-200 bg-white/90 p-4 xl:flex-none">
          <span class="inline-flex size-16 shrink-0 items-center justify-center rounded-lg bg-red-600 text-3xl text-white"><i class="fa-solid fa-ban"></i></span>
          <div>
            ...
            <small class="mt-1 block text-base font-bold text-slate-500">registros anulados</small>
          </div>
        </article>
        <article class="flex min-w-72 flex-1 items-center gap-4 rounded-lg border border-slate-200 bg-white/90 p-4 xl:flex-none">
    ...
    <section class="datatable-card config-table-card min-w-0 rounded-lg border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
      <div class="flex min-w-0 flex-col gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p class="mb-0 mt-1 text-lg text-slate-500">Detalle de cliente, valores, abonos y acceso rapido al credito.</p>
        </div>
        <span class="inline-flex max-w-full self-start rounded-full bg-red-50 px-3 py-2 text-base font-extrabold text-red-700 sm:shrink-0">$<?php echo number_format($totalCredito, 0, ',', '.'); ?> en cartera anulada</span>
      </div>

      <div class="min-w-0 overflow-x-auto">
        <table id="tablaCreditosAnulados" class="display responsive nowrap tabla datatable-table" width="100%">
          <thead>
            <tr>
              <th>ID</th>
              <th>Fecha</th>
